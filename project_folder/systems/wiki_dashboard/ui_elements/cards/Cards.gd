...
@export var article: Resource:Resource = Article.new()
...
func _ready():
	...
	var _x = self.connect("open_card_reader",Callable(card_manager,"open_card_reader"))
...
func _on_Background_gui_input(event):
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_MASK_LEFT and event.doubleclick:
...
func _input(event):
	if event is InputEventMouseButton:	
		if event.button_index == MOUSE_BUTTON_MASK_RIGHT and event.doubleclick:
