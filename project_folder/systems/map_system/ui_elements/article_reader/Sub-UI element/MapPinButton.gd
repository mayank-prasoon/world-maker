...
func _ready():
	...
	if map_pin_resource.pin_symbol_template.template_texture != "":
		$HBoxContainer/TextureRect.texture = ImageHandler.load_image_texture(map_pin_resource.pin_symbol_template.template_texture, Texture2D.FLAG_ANISOTROPIC_FILTER)
